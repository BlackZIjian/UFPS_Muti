			...
			else
			{
				switch(FilterMode)
				{
					...
					case MessageFilterBehavior.Silent:
						PhotonMessageInfo mes = new PhotonMessageInfo(null,0,null);
						AddChatMessage(GetFormattedPlayerName(PhotonNetwork.player.ID) + m_InputLine, mes);
	...
	public static void AddMessage(string message, bool broadcast = true)
	{
		...
		if (m_Chat != null)
		{
			if (broadcast && (PhotonNetwork.connectionStateDetailed == ClientState.Joined))
				...
				m_Chat.AddChatMessage(message, new PhotonMessageInfo(null,0,null));
		}
