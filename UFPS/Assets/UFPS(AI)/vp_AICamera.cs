...
public class vp_AICamera : vp_Component
{

	// character controller of the parent gameobject
	public vp_AIController AIController = null;
...
// camera position
	...
	vp_AIPlayerEventHandler m_Player = null;
	vp_AIPlayerEventHandler Player
	{
		get
		{
			if (m_Player == null)
			{
				if (EventHandler != null)
					m_Player = (vp_AIPlayerEventHandler)EventHandler;
			}
			return m_Player;
		}
	}
	...
	protected override void Awake()
	{
		...
		AIController = Root.GetComponent<vp_AIController>();
		...
		Camera.cullingMask = LayerMask.GetMask("None");
		Camera.depth = 0;
		...
		foreach (Transform t in Transform)
		{
			...
			if (weaponCam != null)
			{
				...
				weaponCam.cullingMask = LayerMask.GetMask("None");
				weaponCam.depth = 1;
	...
	protected override void LateUpdate()
	{
		...
		m_Transform.position = AIController.SmoothPosition;
	...
	public virtual void TryCameraCollision()
	{
		...
		m_CameraCollisionStartPos = AIController.Transform.TransformPoint(0, PositionOffset.y, 0)
			- (m_Player.IsFirstPerson.Get() ? Vector3.zero : (AIController.Transform.position - AIController.SmoothPosition));	// this alleviates stuttering to some extent in third person
		...
		m_CameraCollisionEndPos = Transform.position + (Transform.position - m_CameraCollisionStartPos).normalized * AIController.CharacterController.radius;
		...
		if (Physics.Linecast(m_CameraCollisionStartPos, m_CameraCollisionEndPos, out m_CameraHit, vp_Layer.Mask.ExternalBlockers))
		{
			...
			{
				Transform.position = m_CameraHit.point - (m_CameraHit.point - m_CameraCollisionStartPos).normalized * AIController.CharacterController.radius;
	...
	protected virtual void UpdateBob()
	{
		...
		m_BobSpeed = ((BobRequireGroundContact && !AIController.Grounded) ? 0.0f : AIController.CharacterController.velocity.sqrMagnitude);
	...
	protected virtual void UpdateSwaying()
	{
		...
		Vector3 localVelocity = Transform.InverseTransformDirection(AIController.CharacterController.velocity * 0.016f) * Time.timeScale;
