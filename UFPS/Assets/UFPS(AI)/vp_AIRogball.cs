...
public class vp_AIRogball : MonoBehaviour
{
	...
	protected vp_AIBodyAnimator m_AIBodyAnimator = null;				// UFPS body animator
	protected vp_PlayerEventHandler m_Player = null;				// player event handler
	protected vp_AICamera m_AICamera = null;						// UFPS vpFPCamera, NOTE: only used on local player
	...
	public vp_AICamera AICamera
	{
		get
		{
			if (m_AICamera == null)
				m_AICamera = transform.root.GetComponentInChildren<vp_AICamera>();
			return m_AICamera;
		}
	}
	...
	protected vp_AIBodyAnimator AIBodyAnimator
	{
		get
		{
			if (m_AIBodyAnimator == null)
				m_AIBodyAnimator = GetComponent<vp_AIBodyAnimator>();
			return m_AIBodyAnimator;
		}
	}
	...
	protected virtual void UpdateDeathCamera()
	{
		...
		AICamera.Transform.position = HeadBone.transform.position;
		...
		{
			AICamera.Transform.localEulerAngles = m_CameraFreezeAngle =
			new Vector3(
	...
	public virtual void SetRagdoll(bool enabled = true)
	{
		...
		if(AIBodyAnimator != null)
			AIBodyAnimator.enabled = !enabled;
		if(Controller != null)
